...
创建时间：2014-06-06 15:10:44
生成配置：<root><Page title="股票" id="listStock" /><Model><field title="股票编号" key="true" search="true">stockcode</field><field title="股票名称" search="true">stockname</field><field title="股票交易所编号" key="true" search="true">stockexchangecode</field><field title="描述">stockremark</field></Model><Store ProxyUrl="listStock.action" /><Add RequestUrl="addStock.action" /><Edit RequestUrl="updateStock.action" /><Delete RequestUrl="deleteStock.action"></Delete><DeleteAll RequestUrl="deleteStockAll.action" /></root>
 -->
...
数据字段名
*/
Ext.define('listStock.baseModel', {
    ...
    fields: [
     { name: 'stockcode' },   //股票编号
     { name: 'stockname' },   //股票名称
     { name: 'stockexchangecode' },   //股票交易所编号
     { name: 'stockremark' }   //描述

    ],
   
    idProperty : 'stockcode'
});

/*----------------- 定义数据Store区域 -----------------*/
var thisStore = Ext.create('Ext.data.Store', {
   	model:'listStock.baseModel',
	autoLoad:false,
	pageSize:itemsPerPage,
	proxy:{
		type:'ajax',
		url:'listStock.action',
		reader:{
			type:'json',
			root:'root',
			totalProperty: 'total'
		...
		Ext.apply(store.proxy.extraParams,
			{
				...
				s_stockexchangecode : Ext.getCmp('s_stockexchangecode').getValue()   //检索项目：股票交易所编号
			}
		);
	}
...
{
	var defaultNewRecord = new  listStock.baseModel();
	var addWindow = CreateDetailWindow('新建','addStock.action', defaultNewRecord);	
	addWindow.show();
}
...
function editGridItem()
{
	...
	if (editSelModel.hasSelection()) {
        ...
        if(selectedEditItem){
        	var editWindow = CreateDetailWindow('编辑','updateStock.action', selectedEditItem);
		...
		Ext.Msg.confirm("警告", "确定要删除吗？", 
			function (button) {
    			if (button == "yes") 
    			{
        			...
        			Ext.each(selected, function (item) 
        			{
        				var deleteID = item.data.stockcode;
        				if(deleteID){
            				Ids = Ids + '"' + deleteID + '",';
        				}
        			});
...
//更新全部数据
function refreshGridAllItem()
{
	Ext.Msg.confirm("警告", "确定要更新全部数据吗？", 
			function (button) {
    			if (button == "yes") 
    			{
    				Ext.Ajax.request(
	        			{
	        			 	url:'refreshStockAll.action',
	        			 	method : 'POST',
	        			 	// timeout : 2000,//默认30秒 
	        			 	success : function(response, opts) {
	        			 		var success = Ext.decode(response.responseText).success;
...
function getItemDayDateFromSina() { 
	...
	if (selModel.hasSelection()) 
	{
   		...
   		Ext.Ajax.request(
   			{
   			 	url:'getStockDayDateFromSina.action',
   			 	params :{
   			 		deleteID:Ids
   			 	},
   			 	method : 'POST',
...
function CreateDetailWindow(title,okAction, defaultData)
{
	...
	if(defaultData){
		...
		showForm.getComponent('f_stockremark').setValue(defaultData.get('stockremark'));
	...
	var okButton = Ext.create('Ext.Button', {
		...
		handler:function(button, e, eOpts ){
			...
			var stockremark = showForm.getComponent("f_stockremark").getValue("");
