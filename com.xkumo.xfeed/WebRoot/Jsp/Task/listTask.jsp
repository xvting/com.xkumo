...
创建时间：2014/9/13 14:47:16
生成配置：<root><Page title="任务" id="SendStatisitcs" /><Model><field title="任务ID" key="true" search="true" dataType="string">taskID</field><field title="任务名" search="true" dataType="string">taskName</field><field title="任务描述" dataType="string">taskDescription</field><field title="执行中" dataType="boolean">executing</field><field title="最新执行的开始时间" dataType="date">lastStartTime</field><field title="最新执行的结束时间" dataType="date">lastFinishTime</field></Model><Store ProxyUrl="listTask.action" /><Add RequestUrl="AddTask.Action" /><Edit RequestUrl="EditTask.Action" /><Delete RequestUrl="deleteTaskForUUID.action"></Delete><DeleteAll RequestUrl="deleteTask.action" /></root>
 -->
...
数据字段名
*/
Ext.define('SendStatisitcs.baseModel', {
    ...
    fields: [
     ...
     { name: 'taskName',type:'string' },   //任务名
     { name: 'taskDescription',type:'string' },   //任务描述
     { name: 'executing',type:'boolean' },   //执行中
     { name: 'executeProgress',type:'int' },   //执行中进度
     { name: 'executeProgressDescription',type:'string' },   //执行中进度描述
     
     { name: 'lastStartTime',type:'date' },   //最新执行的开始时间
     { name: 'lastFinishTime',type:'date' }   //最新执行的结束时间
...
var thisStore = Ext.create('Ext.data.Store', {
   	model:'SendStatisitcs.baseModel',
	autoLoad:false,
		...
		Ext.apply(store.proxy.extraParams,
			{
				...
				s_taskName : Ext.getCmp('s_taskName').getValue()   //检索项目：任务名
			}
		);
	}
...
var toolBarGrid = Ext.create('Ext.toolbar.Toolbar',{
		items:[
         		...
         		,
         		{
					text:'运行',
					icon:iconPlay,
					handler : executeGridItem
         		}
         		,
         		{
					text:'终止',
					icon:iconPause,
					handler : terminateGridItem
         		}
         		//,
         		//,
         		//{
...
{
	var defaultNewRecord = new  SendStatisitcs.baseModel();
	var addWindow = CreateDetailWindow('新建','AddTask.Action', defaultNewRecord);	
	addWindow.show();
}
...
function editGridItem()
{
	...
	if (editSelModel.hasSelection()) {
        ...
        if(selectedEditItem){
        	var editWindow = CreateDetailWindow('编辑','EditTask.Action', selectedEditItem);
...
function executeGridItem() { 
	...
	if (selModel.hasSelection()) 
	{
		Ext.Msg.confirm("警告", "确定要运行选中的任务吗？", 
			function (button) {
    			if (button == "yes") 
    			{
        			var selected = selModel.getSelection();
        ...
        Ext.Msg.alert("错误", "没有任何行被选中，无法执行操作！");
    }
}

//终止任务
function terminateGridItem() { 
...
function CreateDetailWindow(title,okAction, defaultData)
{
		...
		items:[
			...
			},
			{
		        fieldLabel: '执行中描述:',
             	labelAlign:'right',
				allowBlank:false,
             	id: 'f_executeProgressDescription'
			},
			{
		        fieldLabel: '最新执行的开始时间:',
             	labelAlign:'right',
				allowBlank:false,
             	id: 'f_lastStartTime'
			},
			{
		        fieldLabel: '最新执行的结束时间:',
             	labelAlign:'right',
				allowBlank:false,
             	id: 'f_lastFinishTime'
			}
	...
	if(defaultData){
		...
		showForm.getComponent('f_executing').setValue(defaultData.get('executing'));
		showForm.getComponent('f_executeProgressDescription').setValue(defaultData.get('executeProgressDescription'));
		showForm.getComponent('f_lastStartTime').setValue(defaultData.get('lastStartTime'));
		showForm.getComponent('f_lastFinishTime').setValue(defaultData.get('lastFinishTime'));
	...
	var okButton = Ext.create('Ext.Button', {
		...
		handler:function(button, e, eOpts ){
			...
			var executing = showForm.getComponent("f_executing").getValue("");
			var lastStartTime = showForm.getComponent("f_lastStartTime").getValue("");
			var lastFinishTime = showForm.getComponent("f_lastFinishTime").getValue("");
...
var listGrid = Ext.create('Ext.grid.Panel',{
                    ...
                    [
                        ...
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'executeProgress',
                            text: '执行进度',
                            width:80
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'executeProgressDescription',
                            text: '执行进度描述',
                            width:180
                        },
                    	{
